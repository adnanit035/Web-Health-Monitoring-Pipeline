...
class Sprint3AdnanStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # passing bucket name to lambda as enviornment variable.
        WH_lambda.add_environment("AdnanBucket", bucket.bucket_name)
        #############################################################################
        ...
        #############################################################################
        # Create an alarm if the Lambda function reaches duration threshold
        duration_alarm = cloudwatch_.Alarm(self, "AdnanLambdaDurationAlarm",
                                           comparison_operator=cloudwatch_.ComparisonOperator.GREATER_THAN_THRESHOLD,
                                           threshold=4000,
                                           evaluation_periods=1,
        ...
        invokation_alarm = cloudwatch_.Alarm(self, "AdnanLambdaInvokationAlarm",
                                             comparison_operator=cloudwatch_.ComparisonOperator.LESS_THAN_THRESHOLD,
                                             threshold=1,
                                             evaluation_periods=1,
        ...
        version_alias = lambda_.Alias(self,
                                      "alias",
                                      alias_name="prod",
                                      version=WH_lambda.current_version
                                      )

        # rollback: version control through code deploy
        codedeploy.LambdaDeploymentGroup(self,
                                         "AdnanDeploymentGroup",
                                         alias=version_alias,
                                         deployment_config=codedeploy.LambdaDeploymentConfig.LINEAR_10_PERCENT_EVERY_1_MINUTE,
                                         alarms=[duration_alarm, invokation_alarm]
                                         )
        #############################################################################
        # ******* End of Set Rollback: Version Control Through Code Deploy **********
        #############################################################################
    ...
    def create_role(self, role_id, description, policy):
        ...
        Args:
            role_id: desired role id
            description: description of the role
            policy: policy to be attached to the role
